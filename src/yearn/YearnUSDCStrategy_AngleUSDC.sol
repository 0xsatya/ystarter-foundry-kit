 ...
 *Submitted for verification at Etherscan.io on 2022-06-24
 Address: 0x7C2b9DB2Ae5aCC6fAC2Fd6cE9b01A5EB4bDD1309
 */
...
interface IStableMasterFunctions {
    ...
    function getStocksUsers()
        external
        view
        returns (uint256 maxCAmountInStable);
...
contract yToken is IERC20, BaseWrapper {
    ...
    function symbol() external view returns (string memory) {
        return
            string(
                abi.encodePacked("y", IERC20Metadata(address(token)).symbol())
            );
    }
    ...
    function permitAll(VaultAPI[] calldata vaults, bytes[] calldata signatures)
        public
    {
    ...
    function migrate(VaultAPI[] calldata vaults, bytes[] calldata signatures)
        external
        returns (uint256)
    {
        _permitAll(msg.sender, vaults, signatures);
    ...
    function revokeAll(VaultAPI[] calldata vaults, bytes[] calldata signatures)
        external
    {
        require(vaults.length == signatures.length);
        for (uint256 i = 0; i < vaults.length; i++) {
            require(
                vaults[i].permit(msg.sender, address(this), 0, 0, signatures[i])
            );
        }
    }
}
...
contract Strategy is BaseStrategy {
    ...
    IERC20 public constant angleToken =
        IERC20(0x31429d1856aD1377A8A0079410B297e1a9e214c2);
    IStableMaster public constant angleStableMaster =
        IStableMaster(0x5adDc89785D75C86aB939E9e15bfBBb7Fc086A87);
    ...
    address public constant treasury =
        0x93A62dA5a14C80f265DAbC077fCEE437B1a0Efde; // To change this, migrate
    ...
    constructor(
        ...
        _initializeStrategy(_sanToken, _sanTokenGauge, _poolManager);
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (_tokensAvailable > 0) {
            uint256 _tokensToGov = (_tokensAvailable * percentKeep) / MAX_BPS;
            if (_tokensToGov > 0) {
        ...
        if (_amountInSanToken > _sanTokenBalance) {
            sanTokenGauge.withdraw(
                Math.min(
                    ...
                    balanceOfStakedSanToken()
                )
            );
        }

        withdrawFromStableMaster(
            Math.min(_amountInSanToken, balanceOfSanToken())
        );
    }
    ...
    function claimableProfitInUsdt() public view returns (uint256) {
        ...
        uint256 _claimableRewards = sanTokenGauge.claimable_reward(
            address(this),
            address(angleToken)
        );

        if (_claimableRewards < 1e18) {
            // Dust check
            return 0;
        }
    ...
    function getSanRate() public view returns (uint256) {
        (, , , , , uint256 _sanRate, , , ) = IStableMaster(angleStableMaster)
